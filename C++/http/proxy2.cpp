...
// otool -L http2
//  g++ -std=c++11 -Wall -pedantic -O3 -Werror=vla -lpthread -levent -I/usr/local/include -L/usr/local/lib -o http2 base64.cpp proxy2.cpp = dynamic
//  g++ -std=c++11 -Wall -pedantic -O3 -Werror=vla -lpthread -I/usr/local/include /usr/local/opt/libevent/lib/libevent.a -o http2 base64.cpp proxy2.cpp = static
...
void serve_proxy(int fd, short event, void * arg){
	...
	timeout1.tv_sec	= 0;	// 30 секунд
	timeout1.tv_usec	= 300000;	// 0 микросекунд (300 милисекунд)

	// Устанавливаем таймаут
	struct timeval timeout2;
	timeout2.tv_sec	= 0;	// 30 секунд
	timeout2.tv_usec	= 300000;	// 0 микросекунд (300 милисекунд)
...
void client_write(int fd, short event, void * arg){
	...
	if(len < data->ans_size - data->offset){
		...
		if(!data->ssl){
			...
			//
			event_set(&data->ev, fd, EV_READ, clien_proxy, data);
			// Reschedule server event
			// event_add(reinterpret_cast <struct event *> (arg), NULL);
			// Schedule client event
			event_add(&data->ev, NULL);
...
void on_connect(int fd, short event, void * arg){
	...
	// Set timeout
	// event_set(&data->ev, sock, EV_TIMEOUT, client_timeout, data);
	// Reschedule server event
	event_add(reinterpret_cast <struct event *> (arg), NULL);
	// Schedule client event
	...
	for(int i = 1; i < max_works; i++){
		...
		switch(pid[i] = fork()){
			...
			case 0:
				...
				event_set(&evserver_sock, fork_buf.socket, EV_READ, on_connect, &evserver_sock);
