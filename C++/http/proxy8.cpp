...
// otool -L http8
// MacOS X
// g++ -std=c++11 -D_BSD_SOURCE -Wall -pedantic -O3 -Werror=vla -o http8 http.cpp base64.cpp proxy8.cpp -I/usr/local/include -levent = dynamic
// g++ -std=c++11 -D_BSD_SOURCE -Wall -pedantic -O3 -Werror=vla -o http8 http.cpp base64.cpp proxy8.cpp -I/usr/local/include /usr/local/opt/libevent/lib/libevent.a = static
// Linux
// g++ -std=c++11 -Wall -pedantic -O3 -Werror=vla -o http8 http.cpp base64.cpp proxy8.cpp -levent = dynamic
// g++ -std=c++11 -Wall -pedantic -O3 -Werror=vla -o http8 http.cpp base64.cpp proxy8.cpp /usr/lib/x86_64-linux-gnu/libevent.a /usr/lib/x86_64-linux-gnu/5/libstdc++.a = static
// g++ -std=c++11 -Wall -pedantic -O3 -Werror=vla -o http8 http.cpp base64.cpp proxy8.cpp /usr/lib/x86_64-linux-gnu/libevent.a /usr/lib/gcc/x86_64-linux-gnu/4.9/libstdc++.a
	...
	// Если все данные получены
		...
		switch(event){
			...
			case 2: {
				...
				// Выполняем проверку прислали все данные или нет
				else if(http->socClient > -1) {
					// Если данные не могут быть отправлены тогда выходим
					if((len = send(http->socClient, (void *) buffer, len, 0)) <= 0) free_data(&http);
		...
		switch(event){
			...
			case 4: {
				...
				// Если все данные отправлены
					...
					http->evServer = event_new(http->base, http->socServer, EV_TIMEOUT | EV_READ | EV_PERSIST, on_http_read_server, http);
					// Устанавливаем таймаут ожидания запроса в 3 секунды
					struct timeval timeout = KEEP_ALIVE;
					// Активируем события
